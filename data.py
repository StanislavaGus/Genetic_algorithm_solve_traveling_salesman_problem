...
class Matrix():
    ...
    def __init__(self):
        # Шаг 1: Ввод диагональной матрицы расстояний
        data = """
        ...
        # Шаг 2: Преобразование данных в список
        rows = data.strip().split('\n')
        distance_data = [list(map(int, row.split())) for row in rows]

        # Шаг 3: Создание полной матрицы расстояний
        num_cities = len(distance_data) + 1
        self.distance_matrix = np.zeros((num_cities, num_cities), dtype=int)

        # Заполнение верхней половины матрицы
        for i in range(num_cities):
            for j in range(i + 1, num_cities):
                self.distance_matrix[i][j] = distance_data[i][j - i - 1]  # j - i - 1 чтобы корректно индексировать
                self.distance_matrix[j][i] = self.distance_matrix[i][j]  # Симметрия

    def print_Distance_Matrix(self):
        # Шаг 4: Вывод матрицы расстояний
        print("Полная матрица расстояний (транспонированная диагональная):")
        for row in self.distance_matrix:
            print(' '.join(map(str, row)))


coorinates = [(1150.0,  1760.0),
              (630.0,  1660.0),
              (40.0,  2090.0),
              (750.0,  1100.0),
              (750.0,  2030.0),
...
best_path = [1, 28, 6, 12, 9, 26, 3, 29, 5, 21, 2, 20, 10, 4, 15, 18, 14, 17, 22, 11, 19, 25, 7, 23, 8, 27, 16, 13, 24, -1]